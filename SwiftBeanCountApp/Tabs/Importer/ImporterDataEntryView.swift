...
struct ImporterDataEntryView: View {
    ...
    init(importManager: ImportManager) {
        ...
        date = metaData.date
        amount = String(describing: posting.amount) + (posting.price != nil ? " @ \(String(describing: posting.price!))" : "")
    ...
    private func getTransaction(in accountName: AccountName) -> SwiftBeanCountModel.Transaction {
        ...
        let newPosting = Posting(accountName: accountName, amount: posting.amount, price: posting.price)
